...
- (BOOL)register {
	...
	if (status != 0) {
		NSLog(@"Failed to register hot key(name:%@, key:%ld, mod:%ld)\
			status: %d", [self getName], self.keyCode, self.keyMod, status);
		return NO;
	}
...
- (BOOL)parseHotKey:(NSString *)hotKey
	...
	NSArray *keys = [[hotKey lowercaseString] componentsSeparatedByString:@" "];
...
- (NSString *)hotKey2String {
	return [HotKey hotKey2String:self.keyCode keyMod:self.keyMod];
}
...
+ (NSString *)hotKey2String:(NSUInteger)keyCode
	...
	NSString *key = [[HotKeyMap sharedMap].code2Key objectForKey:@(keyCode)];
	if (key != nil) {
