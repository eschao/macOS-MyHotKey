...
- (void)registerAll {
	...
	[manager bind:self.switchWindow];
}
...
- (void)defaultHotKeys {
	...
	self.switchWindow = [[WindowHotKey alloc]
	      initWithName:switchWinName
	       description:switchWinDesc
		   defaultHotKey:switchWinHKey
	            target:self
					    action:@selector(performSwitchWindow)];
}
    ...
    NSLog(@"Next Win: [%ld] %@", i, [nextWin getIdentifier]);
		...
		if (nextWin != focusedWindow) {
  		if ([nextWin isMinimized]) {
  			[nextWin restoreFromMinimized];
  		}
  		
  		if (![nextWin isFocused]) {
  			[nextWin setMainWindow];
  		}
		}
	}
}
