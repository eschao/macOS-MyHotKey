...
- (id)getValue:(AXUIElementRef)element {
	...
	if (AXUIElementCopyAttributeValue(
		element, (__bridge CFStringRef)self.name, &value) == kAXErrorSuccess) {
...
- (BOOL)isSettable:(AXUIElementRef)element {
	...
	AXUIElementIsAttributeSettable(
		element, (__bridge CFStringRef)self.name, &isSettable);
...
- (BOOL)setValueWithNSString:(NSString *)value
	...
	return (AXUIElementSetAttributeValue(
		element, (__bridge CFStringRef)self.name, (__bridge CFTypeRef)value
		) == kAXErrorSuccess);
}
...
+ (id)getValueWithName:(NSString *)name
	...
	CFTypeRef value = nil;
	if (AXUIElementCopyAttributeValue(
		element, (__bridge CFStringRef)name, &value) == kAXErrorSuccess) {
